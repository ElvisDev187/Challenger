...
export default function Admin({ tournoi }: Props) {
  ...
  const teamWithOutGroup = tournoi.teams
    ?.filter((team) => team.pouleId === null)
    .map((team) => {
      return { id: team.id, label: team.name };
    });

  const nbPlan = tournoi?.tours?.[0]?.matchs?.filter(
    (match) => match.arbitre == null
  ).length;
  ...
  const { mutate: MatchAuto, isLoading } = useMutation({
    mutationFn: async () => {
      ...
      toast({
        ...
        description:
          "Patienter quelques instant nous generons les matchs possibles",
      });
    ...
    <Tabs defaultValue="staff" className="w-full relative">
      ...
      <TabsContent value="staff">
        <>
          ...
          <Link
            href={`/tournoi/${tournoi.id}/staff`}
            className={buttonVariants({
              variant: "default",
              class:
                "bg-emerald-400  flex items-center justify-center my-5 gap-1 absolute top-0 right-20",
            })}
          >
            ...
            <>
              ...
              <div className="mt-1 grid grid-cols-3 p-7 w-full gap-5">
                {tournoi.Setting?.mode == "SIMPLE" ? (
                  <>
                    {tournoi.tours?.[0]?.matchs!
                      ?.filter((match) => match.arbitre == null)
                      .map((match) => {
                        return (
                          <label htmlFor={match.id}>
                            <div
                              className="bg-white p-3 relative rounded-sm flex flex-col gap-2 items-center justify-center shadow-md text-sm font-medium text-slate-900 "
                              key={match.id}
                            >
                              <div className="flex gap-2">
                                <p> {match.equipeIn.name}</p>{" "}
                                <p className="text-md">vs</p>{" "}
                                <p>{match.equipeOut.name}</p>
                              </div>
                              <Checkbox
              ...
              <h1 className="font-medium text-lg md:text-xl h-10">
                Matchs futur (
                {tournoi.Setting?.mode == "GROUP"
                  ? 0
                  : tournoi.tours?.[0]?.matchs!.filter(
                      (match) =>
                        match.arbitre != null && match.status == "ATTENTE"
                    ).length}
                )
              </h1>
              <div className="mt-1 grid grid-cols-3 p-7 w-full gap-5">
                      ...
                      .map((match) => {
                        return (
                          ...
                          >
                            ...
                            <p className="flex gaap-1">
                              {match.date} - {match.heure} - {match.terrain}
                            </p>
                          </div>
                        );
                      })}
              ...
              <Button
                disabled={isLoading}
                className="flex items-center justify-center my-5 gap-1 absolute top-0 right-20"
                onClick={async () => {
                  if (
                    tournoi.Setting?.mode == "SIMPLE" &&
                    tournoi?.tours?.length == 0 &&
                    tournoi?.groups?.length == 0
                  ) {
                    MatchAuto();
                    return;
                  }
              ...
              <Button
                disabled={isLoading}
                className={` ${
                  Selmatch ? "flex" : "hidden"
                } items-center justify-center my-5 gap-1 absolute top-0 right-[270px]`}
                onClick={async () => {
                  toggleForMatch(true);
                }}
              >
            ...
            <>
              ...
              <div className="mt-1 grid grid-cols-3 p-7 w-full gap-5">
                {tournoi.teams?.map((team) => {
                  return (
                    <div
                      className="bg-white p-3 rounded-sm flex flex-col shadow-md text-sm font-medium text-slate-900 "
                      key={team.id}
                    >
                      <p> {team.name}</p>
                    </div>
              ...
              <div className="mt-1 grid grid-cols-3 p-7 w-full gap-3">
                {tournoi?.groups?.length! > 0 &&
                  tournoi.groups?.map((group) => (
                    <GroupCard group={group} key={group.id} />
                  ))}
              </div>
              <Button
                className="flex items-center justify-center my-5 gap-1"
                onClick={() => toggle(true)}
              >
                <PlusCircle className="h-6 w-6" />
                Former un groupe
